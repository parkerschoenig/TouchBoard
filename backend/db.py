...
def init_db() -> None:
    ...
    try:
        ...
        # In demo mode always reset the admin password so it can't be permanently changed
        from .demo import DEMO_MODE
        if DEMO_MODE:
            conn.execute(
                "UPDATE user SET password_hash=?, is_default_password=1 WHERE username='admin'",
                (hash_password("touchboard"),),
            )
        defaults = [
            ("theme_style",          "classic"),
            ("theme_font",           "inter"),
